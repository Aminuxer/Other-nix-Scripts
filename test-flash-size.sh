...
echo "
            ***  True size checker for USB sticks v. 0.1  ***
...
while [ $size -gt 1 ]
do
   block=$[ $size * 1048576 / $raw_target_block_size ];
   echo -n "Check size $size Mb, block $block ...";
   dd if=$target_dev of=/var/tmp/Dev_$target_disk-block-$block-ORIGINAL-BACKUP.dd count=1 bs=$raw_target_block_size 2>/dev/null
   random_data=`dd if=/dev/urandom of=/tmp/Dev_$target_disk-block-$block-RND1.dd bs=$raw_target_block_size count=1 2>/dev/null`;
   dd if=/tmp/Dev_$target_disk-block-$block-RND1.dd of=$target_dev count=1 bs=$raw_target_block_size 2>/dev/null
   sync
   dd if=$target_dev of=/tmp/Dev_$target_disk-block-$block-RND2.dd count=1 bs=$raw_target_block_size 2>/dev/null
   dd if=/var/tmp/Dev_$target_disk-block-$block-ORIGINAL-BACKUP.dd of=$target_dev count=1 bs=$raw_target_block_size 2>/dev/null
